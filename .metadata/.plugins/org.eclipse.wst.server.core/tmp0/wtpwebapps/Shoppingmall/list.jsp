...
<%
	String sessionId = (String) session.getAttribute("sessionId");					// 로그인 아이디
	List boardlist = (List) request.getAttribute("boardlist");						// 게시물 리스트
	int total_record = ((Integer) request.getAttribute("total_record")).intValue();	// 전체 게시물 개수
	int pageNum = ((Integer) request.getAttribute("pageNum")).intValue();			// 페이지 개수
	int total_page = ((Integer) request.getAttribute("total_page")).intValue();		// 전체 페이지 개수
%>
<%-- 
	세션ID를 가져오고 게시판 객체 생성. 전체 게시글 개수(total_record), 
	전체 페이지(total_page), 페이지 개수(pageNum)로 대입시킴  
--%>
<html>
<head>
